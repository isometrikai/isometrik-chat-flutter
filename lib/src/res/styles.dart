import 'package:flutter/material.dart';
import 'package:isometrik_chat_flutter/isometrik_chat_flutter.dart';

class IsmChatStyles {
  const IsmChatStyles._();

  static TextStyle w400Grey10 = TextStyle(
      color: IsmChatColors.greyColor,
      fontSize: IsmChatDimens.ten,
      fontWeight: FontWeight.w400,
      fontFamily: IsmChatConfig.fontFamily);
  static TextStyle w400Grey12 = TextStyle(
      color: IsmChatColors.greyColor,
      fontSize: IsmChatDimens.twelve,
      fontWeight: FontWeight.w400,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w400Grey14 = TextStyle(
      color: IsmChatColors.greyColor,
      fontSize: IsmChatDimens.forteen,
      fontWeight: FontWeight.w400,
      fontFamily: IsmChatConfig.fontFamily);
  static TextStyle w600Grey16 = TextStyle(
      color: IsmChatColors.greyColor,
      fontSize: IsmChatDimens.sixteen,
      fontWeight: FontWeight.w600,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w500GreyLight12 = TextStyle(
      color: IsmChatColors.greyColorLight,
      fontSize: IsmChatDimens.twelve,
      fontWeight: FontWeight.w500,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w500GreyLight13 = TextStyle(
      color: IsmChatColors.greyColorLight,
      fontSize: IsmChatDimens.tharteen,
      fontWeight: FontWeight.w500,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w500GreyLight17 = TextStyle(
      color: IsmChatColors.greyColorLight,
      fontSize: IsmChatDimens.seventeen,
      fontWeight: FontWeight.w500,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w400Black13 = TextStyle(
      color: IsmChatColors.blackColor,
      fontSize: IsmChatDimens.tharteen,
      fontWeight: FontWeight.w400,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w400Black12 = TextStyle(
      color: IsmChatColors.blackColor,
      fontSize: IsmChatDimens.twelve,
      fontWeight: FontWeight.w400,
      fontFamily: IsmChatConfig.fontFamily);
  static TextStyle w400Black14 = TextStyle(
      color: IsmChatColors.blackColor,
      fontSize: IsmChatDimens.forteen,
      fontWeight: FontWeight.w400,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w400Black16 = TextStyle(
    color: IsmChatColors.blackColor,
    fontSize: IsmChatDimens.sixteen,
    fontWeight: FontWeight.w400,
    fontFamily: IsmChatConfig.fontFamily,
  );

  static TextStyle w400Black18 = TextStyle(
    color: IsmChatColors.blackColor,
    fontSize: IsmChatDimens.eighteen,
    fontWeight: FontWeight.w400,
    fontFamily: IsmChatConfig.fontFamily,
  );

  static TextStyle w600Black14 = TextStyle(
      color: IsmChatColors.blackColor,
      fontSize: IsmChatDimens.sixteen,
      fontWeight: FontWeight.w600,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w600red16 = TextStyle(
      color: IsmChatColors.redColor,
      fontSize: IsmChatDimens.sixteen,
      fontWeight: FontWeight.w600,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w600Black13 = TextStyle(
      color: IsmChatColors.blackColor,
      fontSize: IsmChatDimens.tharteen,
      fontWeight: FontWeight.w600,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w600Black10 = TextStyle(
      color: IsmChatColors.blackColor,
      fontSize: IsmChatDimens.ten,
      fontWeight: FontWeight.w600,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w400Black10 = TextStyle(
      color: IsmChatColors.blackColor,
      fontSize: IsmChatDimens.ten,
      fontWeight: FontWeight.w400,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w600Black12 = TextStyle(
      color: IsmChatColors.blackColor,
      fontSize: IsmChatDimens.twelve,
      fontWeight: FontWeight.w600,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle typing = TextStyle(
      color: IsmChatColors.greenColor,
      fontSize: IsmChatDimens.twelve,
      fontWeight: FontWeight.w600,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w600Black16 = TextStyle(
      color: IsmChatColors.blackColor,
      fontSize: IsmChatDimens.sixteen,
      fontWeight: FontWeight.w600,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w600Black20 = TextStyle(
      color: IsmChatColors.blackColor,
      fontSize: IsmChatDimens.twenty,
      fontWeight: FontWeight.w600,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w600Black18 = TextStyle(
      color: IsmChatColors.blackColor,
      fontSize: IsmChatDimens.eighteen,
      fontWeight: FontWeight.w600,
      fontFamily: IsmChatConfig.fontFamily);
  static TextStyle w600Black24 = TextStyle(
      color: IsmChatColors.blackColor,
      fontSize: IsmChatDimens.twentyFour,
      fontWeight: FontWeight.w600,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w600Black27 = TextStyle(
      color: IsmChatColors.blackColor,
      fontSize: IsmChatDimens.twentySeven,
      fontWeight: FontWeight.w600,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w400White16 = TextStyle(
      color: IsmChatColors.whiteColor,
      fontSize: IsmChatDimens.sixteen,
      fontWeight: FontWeight.w400,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w400White18 = TextStyle(
      color: IsmChatColors.whiteColor,
      fontSize: IsmChatDimens.eighteen,
      fontWeight: FontWeight.w400,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w500White14 = TextStyle(
      color: IsmChatColors.whiteColor,
      fontSize: IsmChatDimens.forteen,
      fontWeight: FontWeight.w500,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w500White16 = TextStyle(
      color: IsmChatColors.whiteColor,
      fontSize: IsmChatDimens.sixteen,
      fontWeight: FontWeight.w500,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w500Black12 = TextStyle(
      color: IsmChatColors.textColor,
      fontSize: IsmChatDimens.twelve,
      fontWeight: FontWeight.w500,
      fontFamily: IsmChatConfig.fontFamily);
  static TextStyle w500Black14 = TextStyle(
      color: IsmChatColors.textColor,
      fontSize: IsmChatDimens.forteen,
      fontWeight: FontWeight.w500,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w500Black16 = TextStyle(
      color: IsmChatColors.textColor,
      fontSize: IsmChatDimens.sixteen,
      fontWeight: FontWeight.w500,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w600White16 = TextStyle(
      color: IsmChatColors.whiteColor,
      fontSize: IsmChatDimens.sixteen,
      fontWeight: FontWeight.w600,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w600White14 = TextStyle(
      color: IsmChatColors.whiteColor,
      fontSize: IsmChatDimens.forteen,
      fontWeight: FontWeight.w600,
      fontFamily: IsmChatConfig.fontFamily);
  static TextStyle w600White18 = TextStyle(
      color: IsmChatColors.whiteColor,
      fontSize: IsmChatDimens.eighteen,
      fontWeight: FontWeight.w600,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w400White14 = TextStyle(
      color: IsmChatColors.whiteColor,
      fontSize: IsmChatDimens.forteen,
      fontWeight: FontWeight.w400,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w400White13 = TextStyle(
      color: IsmChatColors.whiteColor,
      fontSize: IsmChatDimens.tharteen,
      fontWeight: FontWeight.w400,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w400White12 = TextStyle(
      color: IsmChatColors.whiteColor,
      fontSize: IsmChatDimens.twelve,
      fontWeight: FontWeight.w400,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w400White10 = TextStyle(
      color: IsmChatColors.whiteColor,
      fontSize: IsmChatDimens.ten,
      fontWeight: FontWeight.w400,
      fontFamily: IsmChatConfig.fontFamily);

  static TextStyle w700White10 = TextStyle(
    color: IsmChatColors.whiteColor,
    fontSize: IsmChatDimens.ten,
    fontWeight: FontWeight.w700,
    fontFamily: IsmChatConfig.fontFamily,
  );
  static TextStyle w700White12 = TextStyle(
    color: IsmChatColors.whiteColor,
    fontSize: IsmChatDimens.twelve,
    fontWeight: FontWeight.w700,
    fontFamily: IsmChatConfig.fontFamily,
  );

  static TextStyle w700White14 = TextStyle(
    color: IsmChatColors.whiteColor,
    fontSize: IsmChatDimens.forteen,
    fontWeight: FontWeight.w700,
    fontFamily: IsmChatConfig.fontFamily,
  );

  static TextStyle w700White16 = TextStyle(
    color: IsmChatColors.whiteColor,
    fontSize: IsmChatDimens.sixteen,
    fontWeight: FontWeight.w700,
    fontFamily: IsmChatConfig.fontFamily,
  );
  static TextStyle w700Black10 = TextStyle(
      color: IsmChatColors.blackColor,
      fontSize: IsmChatDimens.ten,
      fontWeight: FontWeight.w700,
      fontFamily: IsmChatConfig.fontFamily);
}
